...
describe('Offer Concurrency Integration', () => {
  ...
  beforeEach(async () => {
    ...
    await Offer.updateOne({ _id: offer._id }, { active_revision_id: revision?._id });
  });

  it('should handle concurrent accept requests gracefully (Double Accept Scenario)', async () => {
    const results = await Promise.allSettled([
      offerService.acceptOffer(offer._id.toString(), seller._id.toString(), 'marketplace'),
    ...
    if (rejected.length > 0) {
      console.log('Rejection reasons:', rejected.map(r => (r as PromiseRejectedResult).reason.message));
    }
    ...
    if (fulfilled.length === 1) {
        ...
        console.warn('⚠️ Both calls failed due to contention. This is safe but unexpected with 50ms stagger.');
    }
  });
});
