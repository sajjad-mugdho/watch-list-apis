...
describe("Order Routes", () => {
  ...
  beforeEach(async () => {
    buyer = await TestFactory.createMockUser({
      email: "buyer@example.com",
      external_id: "buyer_123",
    });
    seller = await TestFactory.createMockUser({
      email: "seller@example.com",
      external_id: "seller_123",
    });
  ...
  describe("POST /api/v1/marketplace/orders/reserve", () => {
    it("should successfully reserve a listing", async () => {
      ...
      const updatedListing = await MarketplaceListing.findById(
        (listing as any)._id,
      );
      expect(updatedListing!.reserved_by_user_id!.toString()).toBe(
        buyer._id.toString(),
      );
    });
