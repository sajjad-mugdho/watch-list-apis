...
export class MarketplaceChannelService {
  ...
  async getChannel(
    ...
    const isMember = await marketplaceChannelRepository.isMember(
      channelId,
      userId,
    );
  ...
  async getChannelByGetstreamId(
    ...
    const channel = await marketplaceChannelRepository.findOne({
  ...
  async archiveChannel(channelId: string, userId: string): Promise<void> {
    ...
    await marketplaceChannelRepository.updateById(channelId, {
      status: "closed",
    } as any);
