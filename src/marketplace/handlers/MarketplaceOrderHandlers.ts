...
export const reserveListing = async (
  ...
  try {
    ...
    const buyer_user_id = (req as any).user?.dialist_id;
    const buyer_clerk_id = (req as any).user!.userId;
...
export const getTokenizationForm = async (
  ...
  try {
    const { id } = req.params;
    const buyer_user_id = (req as any).user?.dialist_id; // ✅ Fixed: use dialist_id to match order.buyer_id
    const buyer_clerk_id = (req as any).user?.userId;
    const buyer_location_country = (req as any).user?.location_country; // US or CA
...
export const uploadTracking = async (
  ...
  try {
    ...
    const seller_clerk_id = (req as any).user?.dialist_id;
...
export const requestRefund = async (
  ...
  try {
    ...
    const requester_user_id = (req as any).user?.dialist_id;
