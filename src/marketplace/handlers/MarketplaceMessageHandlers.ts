...
export const sendMessage = (_platform: any) => async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const { channel_id, text, type = "regular", attachments, custom_data, parent_id } = req.body;
    ...
    const channel = await marketplaceChannelService.getChannelByGetstreamId(channel_id, user._id.toString());
...
export const getChannelMessages = (_platform: any) => async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const channel = await marketplaceChannelService.getChannelByGetstreamId(channelId, user._id.toString());
