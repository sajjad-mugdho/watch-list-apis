  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      if (!channel_id || !text?.trim()) {
        res
          .status(400)
          .json({ error: { message: "channel_id and text are required" } });
        return;
      }
      ...
      const channel = await marketplaceChannelService.getChannelByGetstreamId(
        channel_id,
        user._id.toString(),
      );
...
export const markAsRead =
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const isMember = await MarketplaceListingChannel.exists({
