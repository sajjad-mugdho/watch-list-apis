  ...
  try {
    ...
    if (platform === "marketplace") {
      channel = await MarketplaceListingChannel.findOne({
       ...
       channel = await NetworkListingChannel.findOne({
        getstream_channel_id: channel_id,
        $or: [{ buyer_id: user._id }, { seller_id: user._id }],
      });
    }
...
export const getChannelMessages = (platform: Platform) => async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    let channel: any = null;
    if (platform === "marketplace") {
      channel = await MarketplaceListingChannel.findOne({ getstream_channel_id: channelId, $or: [{ buyer_id: user._id }, { seller_id: user._id }] });
    } else {
      channel = await NetworkListingChannel.findOne({ getstream_channel_id: channelId, $or: [{ buyer_id: user._id }, { seller_id: user._id }] });
    }
