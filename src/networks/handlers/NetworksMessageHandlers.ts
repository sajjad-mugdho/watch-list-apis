...
export const sendMessage = (_platform: any) => async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const channel = await networksChannelService.getChannelByGetstreamId(channel_id, user._id.toString());
    ...
    if (channel.status === "closed") {
      res.status(400).json({ error: { message: "Cannot send messages to a closed channel" } });
      return;
    }
    ...
    try {
      ...
      logger.error("GetStream send failed", { streamError, channel_id, messageId: messageId.toString() });
      ...
      await dbMessage.save();
      logger.warn("Message saved but GetStream delivery pending", { messageId: messageId.toString(), channel_id });
    }
...
export const getChannelMessages = (_platform: any) => async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const channel = await networksChannelService.getChannelByGetstreamId(channelId, user._id.toString());

    if (!channel) {
      res.status(403).json({ error: { message: "Not authorized or not a member of this channel" } });
      return;
    }

    const query: any = { stream_channel_id: channelId, is_deleted: { $ne: true } };
...
export const updateMessage = (_platform: "marketplace" | "networks") => async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const auth = getAuth(req);
    if (!auth?.userId) {
      res.status(401).json({ error: { message: "Unauthorized" } });
    ...
    if (!channelContext || !channelContext.parties.some((p: any) => p.clerk_id === auth.userId)) {
      ...
      (async () => {
        if (message.stream_message_id) {
          try {
            await chatService.ensureConnected();
            const client = chatService.getClient();
            await client.updateMessage({ id: message.stream_message_id, text: text.trim() });
          } catch (err: any) {
...
export const deleteMessage = (_platform: "marketplace" | "networks") => async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const auth = getAuth(req);
    if (!auth?.userId) {
      res.status(401).json({ error: { message: "Unauthorized" } });
...
export const markAsRead = (_platform: any) => async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const auth = getAuth(req);
    if (!auth?.userId) {
      res.status(401).json({ error: { message: "Unauthorized" } });
...
export const markAllAsRead = (_platform: any) => async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const auth = getAuth(req);
    if (!auth?.userId) {
      res.status(401).json({ error: { message: "Unauthorized" } });
...
export const reactToMessage = (_platform: any) => async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const auth = getAuth(req);
    if (!auth?.userId) {
      res.status(401).json({ error: { message: "Unauthorized" } });
      ...
      (async () => {
        if (message.stream_message_id) {
          try {
            await chatService.ensureConnected();
            const client = chatService.getClient();
            const channel = client.channel("messaging", message.stream_channel_id);
            await channel.sendReaction(message.stream_message_id, { type, user_id: auth.userId });
          } catch (err: any) {
...
export const archiveChannel = (_platform: any) => async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const auth = getAuth(req);
    if (!auth?.userId) {
      res.status(401).json({ error: { message: "Unauthorized" } });
