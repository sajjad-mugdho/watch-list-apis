...
import { INetworkListingChannel } from "../models/NetworkListingChannel";
import {
  channelService,
  CreateChannelParams,
  GetChannelsParams,
...
export class NetworksChannelService {
  ...
  async getChannel(
    ...
    const isMember = await networksChannelRepository.isMember(
      channelId,
      userId,
    );
  ...
  async getChannelByGetstreamId(
    ...
    const channel = await networksChannelRepository.findOne({
  ...
  async getChannelsForUser(
    params: Omit<GetChannelsParams, "platform">,
  ...
  async archiveChannel(channelId: string, userId: string): Promise<void> {
    ...
    await networksChannelRepository.updateById(channelId, {
      status: "closed",
    } as any);
