...
pushNotificationQueue.process(async (job) => {
  ...
  for (const deviceToken of tokens) {
    try {
      if (
        ...
        logger.debug("[MOCK] Sending FCM", {
          tokenPrefix: deviceToken.token?.slice(0, 8),
          hasTitle: !!title,
          hasBody: !!body,
        });
        results.fcm++;
        ...
        logger.debug("[MOCK] Sending APNs", {
          tokenPrefix: deviceToken.token?.slice(0, 8),
          hasTitle: !!title,
          hasBody: !!body,
        });
      ...
      if (
        ...
        logger.warn("Deactivated invalid push token", {
  ...
  if (results.fcm > 0 || results.apns > 0) {
    logger.debug(
      "[DEPRECATED] Notification update disabled - use platform-specific notification service",
      { notificationId },
    );
