...
export interface IMarketplaceListingChannelModel
  extends Model<IMarketplaceListingChannel> {
...
// ----------------------------------------------------------
// Marketplace Listing Channel Schema
// ----------------------------------------------------------
const marketplaceListingChannelSchema = new Schema<IMarketplaceListingChannel>(
  {
    listing_id: {
...
// Indexes
marketplaceListingChannelSchema.index(
  { listing_id: 1, buyer_id: 1 },
  { unique: true }
);
marketplaceListingChannelSchema.index({ "last_offer.status": 1 });
marketplaceListingChannelSchema.index({ buyer_id: 1, updatedAt: -1 });
marketplaceListingChannelSchema.index({ seller_id: 1, updatedAt: -1 });

// Instance methods
marketplaceListingChannelSchema.methods.isOfferExpired = function (): boolean {
  if (!this.last_offer || !this.last_offer.expiresAt) return false;
  return this.last_offer.expiresAt <= new Date();
};

marketplaceListingChannelSchema.methods.hasActiveOffer = function (): boolean {
  return (
...
marketplaceListingChannelSchema.methods.getUserRole = function (
  userId: string | Schema.Types.ObjectId
): "buyer" | "seller" | null {
...
marketplaceListingChannelSchema.methods.supersedeLastOffer = function (): void {
...
marketplaceListingChannelSchema.methods.resolveLastOffer = async function (
...
// Static methods
marketplaceListingChannelSchema.statics.findByListingAndBuyer = function (
...
marketplaceListingChannelSchema.statics.findByUserId = function (
  userId: string | Schema.Types.ObjectId,
  role?: "buyer" | "seller"
) {
  if (role === "buyer")
...
marketplaceListingChannelSchema.statics.findActiveOffersForSeller = function (
...
// Model - Marketplace
// ----------------------------------------------------------
export const MarketplaceListingChannel = mongoose.model<
  IMarketplaceListingChannel,
  IMarketplaceListingChannelModel
>(
  "MarketplaceListingChannel",
  marketplaceListingChannelSchema,
