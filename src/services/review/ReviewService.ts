...
export class ReviewService {
  constructor() {
    // TODO: Inject platform-specific notification service when needed
  }
  ...
  async createReview(input: CreateReviewInput): Promise<IReview> {
    ...
    if (!reviewableStatuses.includes(order.status)) {
      throw new Error(
        `Cannot review order with status: ${order.status}. Order must be delivered or completed.`,
      );
    }
    ...
    logger.debug(
      "[DEPRECATED] Review notification disabled - use platform-specific notification service",
      { target_user_id, type: "review_received" },
    );
    /*
    try {
   ...
   */
  async atomicIncrementUserStats(
    userId: string,
    rating: number,
    role: ReviewRole,
  ): Promise<void> {
    const roleField =
      role === "buyer"
        ? "stats.review_count_as_buyer"
        : "stats.review_count_as_seller";

    await User.findByIdAndUpdate(userId, [
      {
        $set: {
          "stats.rating_count": {
            $add: [{ $ifNull: ["$stats.rating_count", 0] }, 1],
          },
          "stats.rating_sum": {
            $add: [{ $ifNull: ["$stats.rating_sum", 0] }, rating],
          },
  ...
  async sendReviewReminder(orderId: string): Promise<void> {
    ...
    if (hasReviewed) {
      logger.info("Buyer already reviewed, skipping reminder", {
        orderId,
        buyerId,
      });
      return;
    ...
    // TODO: Use platform-specific notification service
    logger.debug(
      "[DEPRECATED] Review reminder notification disabled - use platform-specific notification service",
      { buyerId, type: "review_reminder" },
    );
    /*
    try {
      await this.notificationService.create({
