...
Related APIs: Networks Consolidated User Profile, Offers, Orders, Social Inbox, Support Count, News
...
<Home Open>
  -> Step 1: Load consolidated networks profile payload
...
4.1 Consolidated Home/Profile Payload
...
- Header identity
- Verification badge
- Top-level activity card counts (orders/offers/wishlist/reference checks/tickets)
...
- Treat consolidated profile payload as a mandatory block.
- Compute count badges from list payloads where needed.

Example:

type HomeState = {
profileBundle: any | null;
...
api.get('/api/v1/networks/user/profile')
...
- Expected behavior: Display first-use empty states with zero counts.
...
Version: 1.0
Last Updated: March 2026
Related APIs: Networks Consolidated User Profile, Orders, Offers, Favorites, Support
...
4.1 Consolidated Profile/Activity Header
GET /api/v1/networks/user/profile
...
- Header data
- Verification banner
- Primary activity card counts

  4.2 Activity Lists (Optional Detail Sources)
  GET /api/v1/networks/orders?type=buy&status=reserved&limit=20&offset=0
  GET /api/v1/networks/offers?type=received&status=active&limit=20&offset=0
...
- Detailed list views and secondary badge reconciliation
...
- Fetch consolidated profile bundle first.
...
- Description: consolidated profile payload is slower than activity list calls.
- Expected behavior: Keep list cards visible and show skeleton in header only.
